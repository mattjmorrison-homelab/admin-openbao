run "secrets_use_renamed_k8s_argocd_app_prefix" {
  ...
  assert {
    ...
    error_message = "local.secrets should no longer have an entry with app = \"homelab-argocd\" -- every consumer has repointed to k8s-argocd and the old entry was retired (admin-openbao#27/#28)"
  }
}
